...
export interface ContextSnapshotPayload {
	...
	/** Top-level symbols (functions, classes) visible in the active file. */
	openSymbols: string[];
	/** Up to 5 recently accessed file paths, newest first. */
	recentFiles: string[];
	/** Active error and warning messages from the Problems panel. */
	diagnostics: string[];
	/** The GitHub remote owner/repo (e.g. "acme/api") to scope searches. */
...
export const saveSnapshot = async (
	...
	try {
		...
		console.log(
			`[Firestore] Saved latest snapshot for user ${userId}`,
		);
	} catch (error) {
		console.error(
		...
		throw new Error(
			"Failed to persist state snapshot to database.",
		);
	}
};
